...
function ListingPhoto({ name, fichier, alt, id }) {
  ...
  const handleDelete = async (photoId) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("le photo n'existe pas");
      }
    ...
    <>
        ...
        <div className=" bg-blue-200 rounded text-white border border-blue-900 p-5">
          ...
          <Modal.Body className="text-center p-4">
            <p className="text-lg font-medium text-blue-900 ">
              Êtes-vous sûr de vouloir supprimer le photo{" "}
      ...
      <li className="grid bg-white md:grid-cols-5 grid-cols-2 border border-blue-900 p-2 rounded m-3">
        ...
        <div className="mx-auto hidden md:block">
          <span className="text-sm font-bold text-blue-900">fichier: </span> {fichier}
        </div>
        <div className="mx-auto hidden md:block">
          <span className="text-sm font-bold text-blue-900">alt: </span>
          {alt}{" "}
        </div>
        <div className="mx-auto">
        ...
        <div>
        <Link className="text-sm font-bold text-blue-900" to={`/modifierphoto/${id}`}>
            Modifier
          </Link>
        </div>
      </li>
